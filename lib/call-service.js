...
const CallService = (version, settings) => {
  ...
  const apiBase = 'https://api.context.io/'
  const versionBase = apiBase + version + '/'
  ...
  return {
    ...
    sendRequest(options) {
      ...
      if (options.url.indexOf(versionBase) === -1) {
        options.url = versionBase + options.url
      }

      // Check to see if this is an app-level call
      if (options.url.indexOf(versionBase + 'app/') !== -1) {
        options.url = options.url.replace(versionBase, apiBase)
      }
    ...
    getFile(url, params) {
      ...
      return this.sendRequest(options).then(res => {
        ...
        if (res.headers['content-disposition']) {
          filename = res.headers['content-disposition'].split('=')[1].slice(1, -1)
        }
